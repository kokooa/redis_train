...
async function main(): Promise<void> {
  ...
  async function purchaseItem(
    clientInstance: RedisClientType,
    itemKey: string,
    userId: string
  ): Promise<boolean> {
    // WATCH: 키 변경 감시 시작
    ...
    const stockStr = await clientInstance.get(itemKey);
  ...
  // 순차 구매 테스트
  await purchaseItem(client as RedisClientType, "stock:item:1", "user1");
  await purchaseItem(client as RedisClientType, "stock:item:1", "user2");
  ...
  try {
    ...
    console.log("트랜잭션 에러:", (err as Error).message);
  }
