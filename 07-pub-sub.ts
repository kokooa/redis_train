...
async function main(): Promise<void> {
  ...
  // 채널 구독
  let messageCount = 0;
  await subscriber.subscribe("chat:room:1", (message: string, channel: string) => {
    messageCount++;
  ...
  let patternCount = 0;
  // chat:* 패턴으로 모든 채팅방 구독
  await subscriber2.pSubscribe("chat:*", (message: string, channel: string) => {
    patternCount++;
  ...
  for (const channel of ["news:tech", "news:sports", "news:weather"]) {
    await subscriber3.subscribe(channel, (message: string, ch: string) => {
